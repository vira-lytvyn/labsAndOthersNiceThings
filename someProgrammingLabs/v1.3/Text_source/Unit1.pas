...
if(Flag1=1) then
begin
if(MessageBox(0,'Сохранить изменения ?', 'Save file', MB_OKCANCEL)=1) then
begin
N3.Click;
Flag1:=0;
Flag2:=0;
...
Panel3.Caption:='0'+'%';
Panel4.Caption:='WIN';
Panel5.Caption:=''+IntToStr(RichEdit1.Lines.Capacity)+' стр.';
...
begin
MessageBox(0,'Версия 1.3', 'Version', MB_OK)
end;
...
N64.Caption:='Выключить';
Flag2:=1;
end
else
begin
RichEdit1.ReadOnly:=False;
N64.Caption:='Включить';
Flag2:=0;
end;
...
Panel6.Enabled:=False;
Form1.N23.Enabled:=False;
Form1.Caption:='Text Безымянный';
Application.Title:='Безымянный';
end;
...
N29.Caption:='Всегда поверх других  выкл';
Top:=1;
end
else
begin
Form1.FormStyle:=fsNormal;
N29.Caption:='Всегда поверх других  вкл';
Top:=0;
end;
RichEdit1.Lines.Clear;
RichEdit1.Lines.Add(st);
...
begin
SetPriorityClass(ProcessHandle,HIGH_PRIORITY_CLASS);
N69.Caption:='Перенос по словам выкл';
s:=RichEdit1.Text;
FlagChange:=1;
RichEdit1.Clear;
FlagChange:=1;
...
begin
SetPriorityClass(ProcessHandle,HIGH_PRIORITY_CLASS);
N69.Caption:='Перенос по словам вкл';
s:=RichEdit1.Text;
FlagChange:=1;
RichEdit1.Clear;
FlagChange:=1;
...
if(Flag1=1) then
begin
B:=MessageBox(0,'Сохранить изменения ?', 'Save file', MB_YESNOCANCEL);	
if(B=6) then
begin
SetPriorityClass(ProcessHandle,HIGH_PRIORITY_CLASS);
