...
namespace one_neurunus
{
    public partial class Form1 : Form
    {
        ...
        private double[][] readStPairs()
        {
            double[][] patterns = new double[studyPairsGV.RowCount][];
            for (int i = 0; i < studyPairsGV.RowCount; i++)
                patterns[i] = new double[3];

            for (int i = 0; i < studyPairsGV.RowCount; i++)
            {
                for (int j = 0; j < studyPairsGV.ColumnCount; j++)
                {
                    patterns[i][j] = Convert.ToDouble(studyPairsGV[j, i].Value);
                }
            }
        ...
        private void studyBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                double firstW = Convert.ToDouble(Weight1TB.Text);
                ...
                singleNeuron.Run(stPairs, stError, stSpeed, sygma, maxIter, firstW);
        ...
        private void DependenseBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                // read parameters
                int maxIterations = Convert.ToInt32(maxIterTB.Text);
                double sigma = Convert.ToDouble(sygmaTB.Text);
                double studySpeed = Convert.ToDouble(speedTB.Text);
                double error = Convert.ToDouble(errorTB.Text);
                double firstW = Convert.ToDouble(Weight1TB.Text);

                // initialize patterns array
                double[][] stPairs = readStPairs();


                DependencesForm depForm = new DependencesForm();
                depForm.loadParameters(stPairs, studySpeed, sigma, maxIterations, error, firstW);
                depForm.ShowDialog();
            }
            catch (FormatException)
            {
