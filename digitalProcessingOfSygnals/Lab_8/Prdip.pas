...
var i1,j1,spim, lx,ly:byte;
    ...
    r:real48;
    gamma:Real;
    h:array[1..256,1..256] of byte;
    hB:array[1..256,1..256] of Extended;
    D:array[1..256,1..256] of byte;
  begin
  n := 3;
  ...
  k:=StrToInt(Edit1.Text);
     ...
     {FNCh}
            ...
            for j:=1 to 128 do
                begin
                    D[i,j]:=round(sqrt(sqr(i)+sqr(j)));
                    if (D[i,j]<k)then H[i,j]:=1 else H[i,j]:=0;
                end;
         for i:=1 to 128 do
            for j:=129 to 256 do
                begin
                    D[i,j]:=round(sqrt(sqr(i)+sqr(j - 256)));
                    if (D[i,j]<k)then H[i,j]:=1 else H[i,j]:=0;
                end;
         for i:=129 to 256 do
            for j:=1 to 128 do
                begin
                    D[i,j]:=round(sqrt(sqr(i - 256)+sqr(j)));
                    if (D[i,j]<k)then H[i,j]:=1 else H[i,j]:=0;
                end;
         for i:=129 to 256 do
            for j:=129 to 256 do
                begin
                    D[i,j]:=round(sqrt(sqr(i - 256)+sqr(j - 256)));
                    if (D[i,j]<k)then H[i,j]:=1 else H[i,j]:=0;
                end;
            for i:=1 to 256 do
              for j:=1 to 256 do
                begin
                      imre1[i,j]:=H[i,j]*imre[i,j];
                      imim1[i,j]:=H[i,j]*imim[i,j];
                end;
           end;
     ...
     {FVCh}
            ...
            for j:=1 to 128 do
                begin
                    D[i,j]:=round(sqrt(sqr(i)+sqr(j)));
                    if (D[i,j]>0)then hB[i,j]:= 1 / (1 + exp(2 * n * ln(k / D[i,j]))) else hB[i,j]:=0;
                end;
         for i:=1 to 128 do
            for j:=129 to 256 do
                begin
         ...
         for i:=129 to 256 do
            ...
            for i:=1 to 256 do
              for j:=1 to 256 do
                begin
                      imre1[i,j]:=hB[i,j]*imre[i,j];
                      imim1[i,j]:=hB[i,j]*imim[i,j];
                end;
           end;
     6:begin
     {FVCh}
     ...
     {FNCh}
         ...
         for i:=1 to 128 do
            for j:=1 to 128 do
                begin
                    D[i,j]:=round(sqrt(sqr(i)+sqr(j)));
                    if (D[i,j]>0)then hB[i,j]:=1 / (1 + exp(2 * n * ln(D[i,j] / k))) else hB[i,j]:=0;
                end;
         for i:=1 to 128 do
            for j:=129 to 256 do
                begin
...
begin
        Label1.Visible:=True;
        Label1.Caption:='Введіть степінь гамма:';
        Edit1.Visible:=True;
        Edit1.SetFocus;
      imet:=9;
end;
...
var i,j,ki,kj:Integer;            // Лінійний фільтр
    M:array[1..9] of byte;
...
var i,j,ki,kj:Integer;             // Медіанний фільтр 
...
{procedure calckIndividula();
var i, j, count, countInrow, max, zoom :integer;
begin
  count := 0;
  max := 1;
  for i:=1 to 256 do
  begin
    ...
    count := count + countInrow;
    if ( countInrow > max ) then
    begin
      max := countInrow;
    end;
  end;
...
begin
  calckIndividula();
  ShowMessage('Diametr is ', diametr);
end; 

procedure TForm1.N40Click(Sender: TObject);
begin
  calckIndividula();
  ShowMessage('Square is ', square);
end;    }
...
begin
      for i:=1 to 256 do
          for j:=1 to 256 do
          begin
                b[i,j]:=abs(a[i+1,j]-a[i,j])+abs(a[i,j+1]-a[i,j]);
          end;
      for i:=1 to 256 do
          for j:=1 to 256 do
          begin
