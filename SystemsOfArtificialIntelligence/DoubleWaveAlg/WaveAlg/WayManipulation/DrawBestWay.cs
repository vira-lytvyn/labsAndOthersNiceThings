...
namespace WaveAlg.WayManipulation
{
    public class BestWayFinder
    {
        ...
        /// <param name="color">color in what we want draw</param>
        /// <param name="forDraw">list of point what we want Draw</param>
        public void Draw(DataGridView dataMatrix, Color color, IEnumerable<Point> forDraw)
        {
            foreach (var point in forDraw)
            {
                dataMatrix[point.X, point.Y].Style.ForeColor = color;
            }
        }
    }
}
